    ...
    try {
        switch ($step) {
            ...
            case 5: // RUN
                ...
                // 4. Migrate
                if (artisan('migrate --force --seed', $log) !== 0) {
                    throw new RuntimeException('migrate --seed failed — see log below');
                }
...
<div class="wrap">
  <div class="card">
    ...
    <?php elseif ($step === 2): ?>
      <form method="post">
        <div class="row">
          <div><label>Host</label><input name="host" type="text" value="<?= h($data['db']['host'] ?? '127.0.0.1') ?>" required></div>
          <div><label>Port</label><input name="port" type="number" value="<?= h($data['db']['port'] ?? '3306') ?>" required></div>
        </div>
        <label>Database</label><input name="database" type="text" value="<?= h($data['db']['database'] ?? 'coops') ?>" required>
        <div class="row">
          <div><label>Username</label><input name="username" type="text" value="<?= h($data['db']['username'] ?? '') ?>" required></div>
          <div><label>Password</label><input name="password" type="password" value="<?= h($data['db']['password'] ?? '') ?>"></div>
        </div>
        <label style="display:flex;align-items:center;gap:6px;text-transform:none;font-weight:400;">
          <input type="checkbox" name="create" value="1" <?= !empty($data['db']['create']) ? 'checked' : '' ?>>
    ...
    <?php elseif ($step === 3): ?>
      <form method="post">
        ...
        <label>App URL</label>
        <input name="url" type="text" value="<?= h($data['app']['url'] ?? ('http://' . ($_SERVER['HTTP_HOST'] ?? 'localhost'))) ?>" required>
